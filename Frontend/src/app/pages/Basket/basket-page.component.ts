...
export class BasketPage {
  ...
  basketProducts$: Observable<Product[]> = this.basketItems$.pipe(
    map((items) => items.map((item) => item.product).filter((p): p is Product => !!p)),
  );
  ...
  removeItem(basketItem?: BasketItem) {
    ...
    this.basketId$
      .pipe(
        switchMap((id) =>
          this.apiClient.basket_RemoveProductFromBasket(id, basketItem.product!.id!),
        ),
      )
      .subscribe(() => {
  ...
  addItem(productId: number) {
    this.basketId$
      .pipe(switchMap((id) => this.apiClient.basket_AddProductToBasket(id, productId)))
      .subscribe(() => {
        this.refreshBasket$.next();
      });
  }
  ...
  updateQuantity(productId: number, quantity: number) {
    this.basketId$
      .pipe(switchMap((id) => this.apiClient.basket_SetProductQuantity(id, productId, quantity)))
      .subscribe(() => {
        this.refreshBasket$.next();
      });
  }
